...
3. Flashing this board **always** needs the `nxpdebugmbox start-debug-session` + `pyocd -O pack.debug_sequences.disabled_sequences=...` dance (see [KNOWLEDGE.md](KNOWLEDGE.md) section 4) - plain `./build.sh flash` alone still fails on this probe/pyOCD/pack combination, unrelated to any model/RAM issue. Worth eventually baking into `build.sh` itself.
